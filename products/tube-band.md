---
title: "鏡筒バンド"
permalink: /products/tube-band/
noindex: true
sitemap: false
author_profile: false
toc: true
toc_label: "目次"
toc_sticky: true
header:
  teaser: /assets/images/tube-band/product-angle.jpg
gallery_mounted:
  - url: /assets/images/tube-band/mounted-sky2.jpg
    image_path: /assets/images/tube-band/mounted-sky2.jpg
    alt: "汎用経緯台に載せた屈折鏡筒を斜め後方から見たところ"
    title: "斜め後方から"
  - url: /assets/images/tube-band/mounted-side.jpg
    image_path: /assets/images/tube-band/mounted-side.jpg
    alt: "鏡筒バンドとアリガタプレートで経緯台に固定した鏡筒を横から見たところ"
    title: "横から"
  - url: /assets/images/tube-band/mounted-closeup.jpg
    image_path: /assets/images/tube-band/mounted-closeup.jpg
    alt: "経緯台のアリミゾにアリガタプレートを固定した部分のクローズアップ"
    title: "取り付け部のクローズアップ"
gallery_product:
  - url: /assets/images/tube-band/product-angle.jpg
    image_path: /assets/images/tube-band/product-angle.jpg
    alt: "アリガタプレートに2本のバンドを取り付けた状態を斜めから見たところ"
    title: "バンド2本＋アリガタプレート"
  - url: /assets/images/tube-band/product-front.jpg
    image_path: /assets/images/tube-band/product-front.jpg
    alt: "同じ構成を正面から見たところ"
    title: "正面から"
  - url: /assets/images/tube-band/product-plate.jpg
    image_path: /assets/images/tube-band/product-plate.jpg
    alt: "アリガタプレート側から見たところ"
    title: "アリガタプレート側から"
---

専用の三脚にしか載せられない鏡筒を、**汎用の経緯台・赤道儀で使えるようにする**
オーダーメイドの鏡筒バンドです。鏡筒の外径に合わせて一台ずつ設計・製作します。
{: .notice}

{% include figure image_path="/assets/images/tube-band/mounted-sky.jpg" alt="専用設計の鏡筒バンドとアリガタプレートで汎用経緯台に載せた屈折鏡筒" caption="作例：もともと専用三脚にしか取り付けられなかった屈折鏡筒を、専用設計の鏡筒バンドとアリガタプレートで汎用の経緯台に載せられるようにしました。" %}

## 特長

- **手持ちの架台が使えるようになる** — アリガタプレートを介して固定するため、市販のアリミゾを備えた経緯台・赤道儀に載せられます。付属の三脚に縛られません。
- **鏡筒に合わせた一点もの** — 外径（mm）に合わせて設計するため、市販バンドのサイズが合わない鏡筒にも対応できます。
- **PETG-CF（カーボン配合）製** — 軽量で剛性が高く、屋外での温度変化にも強い素材です。
- **鏡筒にやさしい** — 金属バンドのように塗装を削ってしまう心配が少なく、締め付け面をならして接触させます。
- **既存の付属品を活かせる** — 鏡筒に元から付いている部品の位置を避けて設計するため、取り外しや加工をせずに使えます。

## 作例：専用三脚から汎用経緯台へ

この鏡筒はもともと**付属の専用三脚にしか取り付けられず**、手持ちの架台に載せることができませんでした。
そこで鏡筒の外径に合わせた**鏡筒バンド2本とアリガタプレートを専用に設計・製作**し、
アリミゾを備えた市販の経緯台へ取り付けられるようにしています。

**ファインダーは鏡筒に元から付いていたものをそのまま使用**しており、新たに追加したものではありません。
鏡筒バンドはファインダーの位置を避けて配置し、既存の機能を損なわないようにしています。

{% include gallery id="gallery_mounted" caption="鏡筒バンド2本とアリガタプレートで鏡筒を支持し、経緯台のアリミゾに固定しています。" %}

## 単体での構成

バンド2本とアリガタプレートを組み合わせた基本構成です。バンドは上下に分割され、ボルト2本で締め付けて鏡筒を保持します。

{% include gallery id="gallery_product" caption="バンド2本＋アリガタプレートの構成例。色・仕上げはロットにより多少異なります。" %}

## 仕様

| 項目 | 内容 |
|:---|:---|
| 素材 | PETG-CF（カーボン配合PETG） |
| 対応鏡筒外径 | 取付位置での実測値に合わせて設計 |
| 締結 | ボルト＋ナット（上下2点） |
| 構成 | 鏡筒バンド2本＋アリガタプレート（作例の構成） |
| 納期 | 設計・印刷の状況により変動（応相談） |

3Dプリント製のため、大口径・重量級の鏡筒には向きません。小〜中口径の鏡筒でのご使用を想定しています。
{: .notice--warning}

## ご注文にあたって

1. **機材全体の写真をお送りください。** 鏡筒と架台が写った写真を何枚かいただければ、鏡筒の形状や付属品の位置関係を確認できます。
2. **載せたい架台をお知らせください。** お使いの経緯台・赤道儀のアリミゾに合うアリガタプレートで設計します。
3. **こちらで取付箇所を決め、測定箇所をお伝えします。** 写真をもとにバンドを回す位置を定め、「どこを、どの向きで測ってほしいか」を図や写真でご案内しますので、その箇所を実測してお知らせください。
4. **採寸が必要な部品はお送りいただきます。** お手持ちのアリガタプレートに合わせる場合は、現物を発払い（元払い）でお送りください。
5. **製作・お渡し。** 直接受け渡しのほか、発送が必要な場合は着払いで発送します。

価格は [鏡筒バンド 価格表]({{ '/tube-band/' | relative_url }}) をご覧ください。
お問い合わせは [沼のほとり](https://ysmrastro.github.io/numanohotori/) の連絡先（LINE / Instagram）からどうぞ。
{: .notice--info}
